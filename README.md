...
Path Access Control is a [Zoraxy](https://github.com/tobychui/zoraxy) router
plugin that applies an existing Zoraxy IP/CIDR access rule to selected URL
paths. It keeps a host public while protecting sensitive areas such as
...
### Zoraxy plugin system

Open `/#appstore` in the Zoraxy management interface, go to **Settings** and
add the following plugin repository:

```text
https://raw.githubusercontent.com/IloveeCobrakai/zoraxy-official-plugins/refs/heads/main/directories/index2.json
```

Return to the app store and install **Path Access Control** through Zoraxy's
